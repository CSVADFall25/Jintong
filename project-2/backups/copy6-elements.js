...
// ring style config
const RING_GAP_PX = 1;              // fixed radial gap between rings
const MIN_RING_THICKNESS_PX = 3;    // minimum visual thickness per ring
...
function updateFlowers(startDate, endDate) {
  ...
  all.sort((a,b) => a.date - b.date);
    ...
    const avgTd = tdCount ? tdSum/tdCount : 0;
  ...
  let r = constrain(rBase, MIN_FLOWER_R, MAX_FLOWER_R) * FLOWER_SCALE;
    ...
    rects.push({cx, cy, r: usedR, halo, groupIndex: i, entries: all, total});
...
function generateStem() {
  ...
  for (let i = 0; i < rects.length; i++) {
    const f = rects[i];
    // compute min time difference (hours) across group's entries
    let minTD = null;
    const g = groups[f.groupIndex] || [];
    for (let day of g) {
      for (let e of (day.entries||[])) {
        const td = (typeof e.timeDiff === 'number') ? e.timeDiff : parseFloat(e.timeDiff);
        if (!isNaN(td)) minTD = (minTD===null)? td : Math.min(minTD, td);
      }
    }
    if (minTD === null) minTD = 0;
